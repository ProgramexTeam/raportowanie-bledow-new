...
public class EditTicket extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        } else if(status == null){ request.setAttribute("msg", "Nie podano statusu projektu");
        } else if(title == null){ request.setAttribute("msg", "Nie podano ilości sprzedanyc");
        } else if(description == null){ request.setAttribute("msg", "Nie podano opisu ticketu");
        } else {
            boolean done = TicketDAO.editGivenProduct(ticket_id, project_id, status, title, description);
            if(done){
