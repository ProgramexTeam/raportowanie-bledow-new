...
public class GeneralConfigFile {
    ...
    public GeneralConfigFile (ServletContext context) {
        String pathToConfig = ContextOperations.getPathToRoot(context.getRealPath("")) + "\\src\\main\\webapp\\WEB-INF\\config\\config.xml";
    ...
    public void updateConfigFile(ServletContext context, HashMap<String, String> configuration, String pathToConfig){
        try {
            InputStream fXmlFile = context.getResourceAsStream("/WEB-INF/config/config.xml");
            DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
            DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
            Document doc = dBuilder.parse(fXmlFile);
