...
public class HomePageConfigFile {
    private HashMap<String, String> map;
    public HomePageConfigFile(ServletContext context) {
        String pathToConfig = ContextOperations.getPathToRoot(context.getRealPath("")) + "\\src\\main\\webapp\\WEB-INF\\config\\homepage-config.xml";
    ...
    public void updateConfigFile(ServletContext context, HashMap<String, String> configuration, String pathToConfig){
        try {
            InputStream fXmlFile = context.getResourceAsStream("/WEB-INF/config/homepage-config.xml");
            DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
            DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
            Document doc = dBuilder.parse(fXmlFile);
            ...
            System.out.println("An error occurred while updating homepage-config.xml file; HomePageConfigFile.updateConfigFile()" + ex);
        }
    }
}